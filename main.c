...
void *give_command( void* arguments)
{
    ...
    for(int i = 0; i < command_list.num_orders; i++)
    {
        ...
        if(command_list.command[i] == 0)
        {
            ...
            {
                hw2_notify(action = PROPER_PRIVATE_TOOK_BREAK, command_list.privates[m].gid, 0, 0);
            }
            sem_wait(&sem_break);
        }
        else if(command_list.command[i] == 1)
        {
            ...
            for(int m = 0; m < command_list.num_privates; m++)
            {
                hw2_notify(action = PROPER_PRIVATE_CONTINUED, command_list.privates[m].gid, 0, 0);
            }
            sem_post(&sem_break);
        }
        else
        {
            ...
            for(int m = 0; m < command_list.num_privates; m++)
            {
                hw2_notify(action = PROPER_PRIVATE_STOPPED, command_list.privates[m].gid, 0, 0);
            }
            ...
            stop = 1;
            sem_post(&sem_break);
            usleep((orders->time_order[orders->num_orders - 1] - command_list.time_order[i]) * 1000);
            exit(0);
        }
    }
...
void *collect_cigg_butts( void* arguments)
{
    ...
    hw2_notify(action = PROPER_PRIVATE_CREATED, (*private_prop).gid, 0, 0);
    
    for(int num = 0; num < (*private_prop).ng; num++)
    {
        sem_wait(&gatherers_prior[(*private_prop).queue][gatherers_job[(*private_prop).queue]]);
        hw2_notify(action = PROPER_PRIVATE_ARRIVED, (*private_prop).gid, (*private_prop).top_corners[num][0],(*private_prop).top_corners[num][1]);
        for(int j = 0; j < (*private_prop).sj; j++)
        {
            for(int i = 0; i < (*private_prop).si; i++ )
            {
                ...
                while(cig_butts[coord_i][coord_j] > 0)
                {
                    ...
                    hw2_notify(action = PROPER_PRIVATE_GATHERED, (*private_prop).gid, coord_i,coord_j);
                    sem_post(&lock);
                }
            }
        }
        gatherers_job[(*private_prop).queue]++;
        hw2_notify(action = PROPER_PRIVATE_CLEARED, (*private_prop).gid, 0, 0);
    }
    hw2_notify(action = PROPER_PRIVATE_EXITED, (*private_prop).gid, 0, 0);
}
